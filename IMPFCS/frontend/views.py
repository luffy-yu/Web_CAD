...
def foyer(request):
    ...
    if request.user.is_superuser:
        ...
        teamApplications = {
            'teamApplications': [{'teamName': u'游泳队', 'year': 2014, 'month': 1, 'day': 1, 'state': 'rejected'}]}
...
def scores(request):
    scores = {'scores': [
        {'category': '射击', 'name': '奥运会', 'competition': '男子50米步枪3X40', 'rank': '1', 'score': '80', 'time': '0.01s',
         'dist': '2m', 'state': 'rejected'}]}
...
def fff(request):
    ...
    if request.method == 'POST':
        ...
        # print "error"
            ...
            while ts < len(name):
                collection.insert(
                    {'vistorid': vistorid, 'dataid': (ts + 3) / 3, 'pid': int(name[ts + 2]), 'pos_x': int(name[ts]),
                     'pos_y': int(name[ts + 1])})
                ts = ts + 3
                # print "ss"
...
def all_db_setup(curvistorid):
    ...
    while i < num:
        ...
        i = i + 1
        db.device1.insert(
            {"vistorid": curvistorid, "p0": int(sss[0]), "p1": int(sss[1]), "p2": int(sss[2]), "p3": int(sss[3]),
             "p4": int(sss[4])})
    lines = up_file.readline()
    num = 0
    ss = lines.split(" ")
    ...
    while i < num:
        ...
        while t < len(sss) - 1:
            ...
            ssss2 = ssss[1].split(")")
            db.control1.insert(
                {"vistorid": curvistorid, "order": a, "px": int(ssss1[1]), "py": int(ssss2[0]), "psid": i})
            t = t + 1
            a = a + 1
        i = i + 1
    up_file.close()
...
def flow_db_setup():
    ...
    while i < num:
        ...
        i = i + 1
        db.device.insert(
            {"p0": int(sss[0]), "p1": int(sss[1]), "p2": int(sss[2]), "p3": int(sss[3]), "p4": int(sss[4])})
    lines = up_file.readline()
    num = 0
    ss = lines.split(" ")
    if ss[0] == 'path':
        ...
        while t1 < i1:
            if lines[t1] == "(":
                ...
                while num_tmp_id < times:
                    ...
                    num_tmp_id = num_tmp_id + 1
                db.st1.save(
                    {'vistorid': curvistorid, 'dataid': save_id, 'pid': m1, 'pos_x': num_tmp1, 'pos_y': num_tmp2})
                save_id = save_id + 1
                times = 0
                num_tmp1 = 0
                num_tmp2 = 0
